<template>
  <div class="wrapper">
    <navHeader v-if="showSideAndNavBar"></navHeader>
    <navegationmenu v-if="showSideAndNavBar"></navegationmenu>
    
    <router-view></router-view>

    <footer v-if="showSideAndNavBar" class="main-footer">
        <div class="pull-right hidden-xs">
            <b>Versión</b> 2.5.0
        </div>
        <strong>Copyright &copy; {{ new Date().getFullYear() }}-MEM</strong>
    </footer>
  </div>
...
export default {
  ...
  computed: {
    authenticated(){
      let self = this
      return self.user.authenticated ? "wrapper" : ""
    },

    showSideAndNavBar(){
      let self = this
      var sistemaEmpty = _.isEmpty(self.$store.state.sistema)
