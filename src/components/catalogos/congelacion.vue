  ...
  <div class="content-wrapper" v-loading="loading">
    <section class="content-header">
      ...
      <ol class="label breadcrumb bg-gray">
        ...
        <li><i class="fa fa-bug active"></i> Congelación</li>
      </ol>
    </section>
    <br />
            ...
            <div class="box-body">
              <form role="form" autocomplete="off">
                ...
                <label for="tipo_congelacion_id"
                    >Tipo Congelación</label
                  >
                  <multiselect
                    v-model="form.tipo_congelacion_id"
                    @input="getTipocongelacion"
                    :options="tipo_congelacion"
                  ...
                  <FormError
                    :attribute_name="'Tipo de Congelación'"
                    :errors_form="errors"
                  >
                  </FormError>
                </div>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        { key: "tipo_congelacion.nombre", label: "Tipo Congelación", sortable: true },
  ...
  methods: {
    ...
    getPromise() {
      this.loading = true;
      Promise.all([this.getAll(), this.getTipocongelacion()])
        .then(() => {
          this.loading = false;
        })
        .catch((error) => {
    ...
    getAll() {
      ...
      self.$store.state.services.congelacionService
        .getAll()
        .then((r) => {
          self.loading = false;
    ...
    getTipocongelacion() {
      let self = this;
      self.tipo_congelacion = [];
      self.$store.state.services.tipocongelacionService
        .getAll()
        .then((r) => {
          r.data.data.forEach((element) => {
            if (!element.deleted_at) self.tipo_congelacion.push(element);
          });
        })
        .catch((r) => {console.log(r)});
    },
    ...
    mapData(data) {
      ...
      self.form.tipo_congelacion_id = data.tipo_congelacion;
    },
