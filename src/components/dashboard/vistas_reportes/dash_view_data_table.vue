<template>
  <div class="content-wrapper" >
      <section class="content">  
        <div class="row">
          <div class="col">
              <div class="box">
                <div class="row">
                  <!--Activos 2 Cerrados 3 Congelados 4-->
                  <dash_data_table v-if="reporte===2"
                    :reporte="reporte"
                    :pertenece="pertenece"
                    :fieldsActivos="fields"
                    :tipo="tipo"
                    :unidad="unidad"
                    :consolidado="consolidado"
                    :fechaInicial="fecha_inicial"
                    :fechaFinal="fecha_final"
                    :filtroFecha="tipo_reporte"
                    :mes="mes"
                    :anio="anio"
                    :nombre_reporte="nombre_reporte"
                    :nombre_tipo="nombre_tipo"
                  ></dash_data_table>          

                  <dash_data_table v-if="reporte===3"
                    :reporte="reporte"
                    :pertenece="pertenece"
                    :fieldsCerrados="fields"
                    :tipo="tipo"
                    :unidad="unidad"
                    :consolidado="consolidado"
                    :fechaInicial="fecha_inicial"
                    :fechaFinal="fecha_final"
                    :filtroFecha="tipo_reporte"
                    :mes="mes"
                    :anio="anio"
                    :nombre_reporte="nombre_reporte"
                    :nombre_tipo="nombre_tipo"
                  ></dash_data_table>   

                  <dash_data_table v-if="reporte===4"
                    :reporte="reporte"
                    :pertenece="pertenece"
                    :fieldsCongelados="fields"
                    :tipo="tipo"
                    :unidad="unidad"
                    :consolidado="consolidado"
                    :fechaInicial="fecha_inicial"
                    :fechaFinal="fecha_final"
                    :filtroFecha="tipo_reporte"
                    :mes="mes"
                    :anio="anio"
                    :nombre_reporte="nombre_reporte"
                    :nombre_tipo="nombre_tipo"
                  ></dash_data_table>                          
                </div>
              </div> 
          </div>
      </div>
    </section>
  </div>
</template>

<script>
import dash_data_table from "../../dashboard/dash_data_table";
export default {
  name: 'dash_view_data_table',
  components: {
    dash_data_table,
  },
  data () {
    return {
      reporte : null,
      pertenece : null,
      fields:null,
      tipo : null,
      unidad : null,
      consolidado : null,
      abreviatura: null,
      fecha_inicial:null,
      fecha_final:null,
      tipo_reporte:null,
      mes:null,
      anio:null,
      nombre_reporte:null,
      nombre_tipo:null,
      reportes: [
        { id: 2, nombre: "Reporte Expedientes Activos"},
        { id: 3, nombre: "Reporte Expedientes Cerrados"},
        { id: 4, nombre: "Reporte Expedientes Congelados"},
      ],
      tipo_reportes: [
        { id: 1, nombre: "Todos los resultados de expedientes"},// se puede usar si se desea
        { id: 2, nombre: "Del:"},
        { id: 3, nombre: "De:"}
      ],
      meses : [
        "opcion nula","Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio",
        "Agosto","Septiembre","Octubre","Noviembre","Diciembre"
      ]
    }
  },
  created(){
    let self = this
    self.reporte= Number(atob(self.$route.params.reporte));
    self.pertenece = atob(self.$route.params.pertenece);
    self.fields = JSON.parse(atob(self.$route.params.fields));    
    self.tipo = atob(self.$route.params.tipo);
    self.unidad = Number(atob(self.$route.params.unidad));
    self.consolidado = Number(atob(self.$route.params.consolidado));
    self.abreviatura = atob(self.$route.params.abreviatura);

    self.fecha_inicial = atob(self.$route.params.fechaInicial);
    self.fecha_final = atob(self.$route.params.fechaFinal);
    self.tipo_reporte = Number(atob(self.$route.params.tipo_reporte));//tipo reporte referencia a (1: Todo 2: Rango de fehca 3: mensual)
    
    self.mes = Number(atob(self.$route.params.fechaInicial)); //aplica lo mismo para fecha_inicial y final
    self.anio = atob(self.$route.params.fechaFinal); // aplica lo mismo para fecha_inicial y final
    if(self.pertenece === "nothing"){
      self.pertenece = null;
      self.tipo = null;
    }else{
      
      self.pertenece = Number(self.pertenece);
      self.tipo = Number(self.tipo);  
    }
    //valida el nombre del reporte
    if(this.reporte === 2){
      this.nombre_reporte = this.reportes[0].nombre;
    }else if(this.reporte === 3){
      this.nombre_reporte = this.reportes[1].nombre;
    }else if(this.reporte === 4){
      this.nombre_reporte = this.reportes[2].nombre;
    }
    //valida el nombre del tipo de reporte
    if(this.tipo_reporte === 1 ){
      this.nombre_tipo = "";
    }else if(this.tipo_reporte === 2){
      this.nombre_tipo = this.tipo_reportes[1].nombre + " " + this.fecha_inicial + " a " + this.fecha_final;  
    }else if(this.tipo_reporte === 3){
      this.nombre_tipo = this.tipo_reportes[2].nombre + " " + this.meses[this.mes] + " del " + this.anio;
    }

  },
  mounted(){
    $("body").resize()
  },
  filters: {
    upper: function (value) {
      if (!value) return "";
      value = value.toString();
      return value.toUpperCase();
    },
  },
    methods:{
    regresa_dash(){
      console.log("retorno")
      let url = "/dash_default"
      self.$router.push({path:url});
    }
  }
}
</script>
